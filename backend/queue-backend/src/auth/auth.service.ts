...
export class AuthService {
  ...
  async refresh(refreshToken: string | undefined) {
    ...
    if (!user?.isActive || !user.refreshTokenHash) throw new UnauthorizedException('Session is no longer valid');
  ...
  async me(userId: string) {
    return this.prisma.user.findUniqueOrThrow({ where: { id: userId }, select: this.safeUserSelect() });
  }
